    ...
    <>
      <div className="question container">
        ...
        <div className="answers">
          {answers?.map((answer, index) => (<Answer answer={answer} key={index} index={index} onSelectAnswer={answer => dispatch && dispatch({ type: allowedActions.SELECT_ANSWER, payload: answer })} selectedAnswer={selectedAnswer} correctAnswer={correctAnswer} askedHelp={askedHelp} />))}
        </div>

        <div className="action-btns-wrap">

          <div className={`help-btn action-btn ${helpDisabled}`} onClick={() => dispatch && dispatch({ type: allowedActions.ASK_HELP, payload: null })}>
            <span className="action-btn-text">Help</span>
            <span className="help-chances">{helpChances}</span>
          </div>

          <div className="next-btn action-btn" onClick={() => dispatch && dispatch({ allowedActions.NEXT_QUESTION, payload: null })}>
